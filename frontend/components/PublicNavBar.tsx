...
function CategoryIcon({
  ...
  const showPlaceholder = !iconUrl || errored || !loaded;

  return (
    <span className={`relative flex ${dim} shrink-0 items-center justify-center rounded-full bg-slate-100`}>
      {iconUrl && !errored && (
        <img
          src={iconUrl}
          alt=""
          className={`${dim} rounded-full p-0.5 object-contain transition-opacity duration-200 ${loaded ? "opacity-100" : "opacity-0"} absolute inset-0`}
          onLoad={() => setLoaded(true)}
          onError={() => setErrored(true)}
        />
      )}
      {showPlaceholder && (
        <span className="text-xs font-bold text-slate-600">{name.charAt(0).toUpperCase()}</span>
      )}
    </span>
  );
}
                ...
                <div className="rounded-xl border border-slate-700 bg-slate-900 py-1.5 shadow-lg">
                  {categories.map((c) => {
                    ...
                    return (
                      ...
                      >
                        <CategoryIcon iconUrl={iconUrl} name={c.name} />
                  ...
                  {categories.map((c) => {
                    ...
                    return (
                      <Link
                        ...
                        onClick={() => setMobileOpen(false)}
                      >
                        <CategoryIcon iconUrl={iconUrl} name={c.name} size="sm" />
                        <span>{c.name}</span>
                      </Link>
                    );
                  })}
