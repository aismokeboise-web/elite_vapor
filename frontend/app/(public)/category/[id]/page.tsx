...
function getCategoryGradient(name: string): string {
  ...
  if (key.includes("disposable")) {
    return "border-rose-200 bg-gradient-to-r from-rose-100 via-slate-50 to-amber-100";
  }
  if (key.includes("accessor")) {
    return "border-indigo-200 bg-gradient-to-r from-indigo-100 via-slate-50 to-sky-100";
  }
  if (key.includes("pod") || key.includes("device")) {
    return "border-emerald-200 bg-gradient-to-r from-emerald-100 via-slate-50 to-teal-100";
  }
  if (key.includes("nicotine") || key.includes("pouch")) {
    return "border-amber-200 bg-gradient-to-r from-amber-100 via-slate-50 to-orange-100";
  }
  if (key.includes("e-liquid") || key.includes("liquid") || key.includes("juice")) {
    return "border-violet-200 bg-gradient-to-r from-violet-100 via-slate-50 to-fuchsia-100";
  }
  if (key.includes("kit") || key.includes("starter")) {
    return "border-sky-200 bg-gradient-to-r from-sky-100 via-slate-50 to-cyan-100";
  }
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    setPage(1);
  }, [search, brandId, id, minPrice, maxPrice, modelId]);
          ...
          <div className="mt-8 flex justify-center">
            <div className="w-full max-w-3xl rounded-3xl border border-dashed border-slate-200 bg-white/80 px-6 py-12 text-center shadow-sm">
              ...
              <p className="mt-2 text-sm text-slate-600">
                We&apos;re still adding items here. In the meantime, you can browse all products or explore other sections.
              </p>
              <div className="mt-6 flex flex-wrap justify-center gap-3">
                <Link
                  href="/products"
                  className="inline-flex items-center rounded-full border border-slate-800 bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 hover:border-slate-700"
                >
                  Browse all products
                </Link>
                <Link
                  href="/"
                  className="inline-flex items-center rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-800 shadow-sm transition hover:bg-slate-50"
                >
                  Back to home
                </Link>
              </div>
