...
export function ProductDetailClient({ id }: { id: string }) {
  ...
  const cat = product.brand.category;
  const catHref = `/category/${cat.slug || cat.id}`;
        ...
        <div className="mx-auto max-w-6xl px-4 py-4 sm:px-6 sm:py-5 lg:px-8">
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
              ...
              <Link href="/" className="text-slate-700 hover:text-slate-900 hover:underline">Home</Link>
              <span aria-hidden className="text-slate-400">/</span>
              <Link href={catHref} className="text-slate-700 hover:text-slate-900 hover:underline">
                {cat.name}
              </Link>
            </div>
        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-12">
          ...
          <div className="space-y-4">
            <div className="aspect-square overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <img src={imageUrl} alt={selectedModel?.name || product.name} className="h-full w-full object-cover" />
            </div>
            {thumbnails.length >= 1 && (
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Product details</h2>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="space-y-1">
                      <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Price</span>
                      <p className="text-2xl font-semibold tracking-tight text-slate-900">${selectedModel.price}</p>
                    </div>
                    <div className="flex flex-col items-end gap-1.5">
                      {selectedModel.is_deal && (
                        <span className="inline-flex items-center rounded-full bg-rose-500 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-white shadow-sm">
                          {selectedModel.deal_text?.trim() || "On Sale"}
                        </span>
                      )}
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {selectedModel.flavors.map((f, i) => {
                        ...
                        return (
                          <span key={f} className={`rounded-lg border px-3 py-1.5 text-sm font-medium ${colors[i % colors.length]}`}>
                            {f}
                          </span>
                        );
                      })}
