...
const modelsReadRouter = Router();

modelsReadRouter.get("/", async (_req: Request, res: Response) => {
  try {
...
modelsReadRouter.get("/:id", async (req: Request, res: Response) => {
  try {
...
export function createModelsWriteRouter(modelImageUpload: Multer): Router {
  ...
  router.post(
    ...
    (req: Request, res: Response, next: () => void) => {
      modelImageUpload.single("image")(req, res, (err: unknown) => {
        if (err) {
          const message = err && typeof (err as any).code === "string" && (err as any).code === "LIMIT_FILE_SIZE"
  ...
  try {
    ...
    const deal_text = (req.body as { deal_text?: unknown }).deal_text;

    const flavors =
      flavorsRaw == null
        ? []
        : Array.isArray(flavorsRaw)
          ? flavorsRaw.map((v: unknown) => String(v).trim()).filter(Boolean)
          : typeof flavorsRaw === "string"
            ? flavorsRaw.split(",").map((s: string) => s.trim()).filter(Boolean)
            : [];
    ...
    const imageUrls =
      imageUrlsRaw == null
        ...
        : Array.isArray(imageUrlsRaw)
          ? imageUrlsRaw.map((v: unknown) => String(v).trim()).filter(Boolean)
  ...
  try {
    ...
    if (is_clearance !== undefined) data.is_clearance = !!is_clearance;
    if (is_deal !== undefined) data.is_deal = !!is_deal;
    if (deal_text !== undefined) data.deal_text = deal_text != null && String(deal_text).trim() !== "" ? String(deal_text).trim() : null;
    if (is_best_seller !== undefined) data.is_best_seller = !!is_best_seller;
    if (is_new !== undefined) data.is_new = !!is_new;
