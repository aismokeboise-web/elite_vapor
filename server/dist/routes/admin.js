...
function createAdminRouter(upload) {
...
<body>
  ...
  <script>
    ...
    // Edit brand form
    router.get("/brands/edit", (_req, res) => {
        res.type("html").send(`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  ...
  <script>
    ...
    router.get("/brands", async (_req, res) => {
        try {
            ...
            const items = brands
                .map((b) => `<li><strong>${b.name}</strong> – id: <code>${b.id}</code></li>`)
                .join("");
            res.type("html").send(`<!DOCTYPE html>
<html lang="en">
<head>
    ...
    router.get("/categories", async (_req, res) => {
        try {
            ...
            const items = categories
                .map((c) => `<li><strong>${c.name}</strong> – id: <code>${c.id}</code></li>`)
                .join("");
            res.type("html").send(`<!DOCTYPE html>
<html lang="en">
<head>
    ...
    router.get("/products", async (_req, res) => {
        try {
            ...
            const items = products
                .map((p) => {
                const modelCount = Array.isArray(p.modelIds) ? p.modelIds.length : 0;
                return `<li>
...
<body>
  ...
  <p><a href="/admin">Back to admin</a></p>
    ...
    router.get("/models/images", (req, res) => {
...
<body>
  ...
  <script>
    ...
    router.post("/models/images", upload.fields([
        ...
        try {
            ...
            const existing = await prisma_1.default.model.findUnique({ where: { id: modelId } });
            if (!existing) {
                res.status(404).send("Model not found.");
                return;
            }
            await prisma_1.default.model.update({
                where: { id: modelId },
                data: { imageUrls },
            });
